...
class UserManager(models.Manager):
    ...
    def validateLogin(self, postData):
        ...
        def allComments(self):
            comments = []
            for comment in self.recieved_comments.all():

                print comment.comment_text
                comments.append(comment.comment_text)
            return self.recieved_comments.all(),

        def otherUsers(self):
            # order by count if possible // doesn't work currently
            return User.objects.exclude(id=self.id).annotate(comment_count=Count('recieved_comments')).order_by('comment_count')
